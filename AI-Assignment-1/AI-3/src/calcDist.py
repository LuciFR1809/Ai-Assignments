...
conn = psycopg2.connect(user="postgres", password="311285", database="Hyderabad_map", host="localhost", port="5432")
cur = conn.cursor()
...
def getEdges(src, dest,  dec_limit=15):
    #if size=='large':
...
def main():
    ...
    adjframe, edgeList, nodes = getEdges(src, dest)


    adjframe.to_pickle('Adjframe.pkl')
